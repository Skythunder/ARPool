/*Creates final calibration matrix with unity interface. Press 'f' for fullscreen followed by 'c' for calibration.
...
int mainOFFUPC()//set to main
{
	...
	for( float i = 11; i >= -11.6; i-=3.77 )
		for( float j = -16; j <= 13.2; j+=3.65 )
			pointBufloc.push_back(Point2f(float( j ), float( i )));
	bool loop = true;
	Mat res;
	bool calib=false;
	int times=0;
	...
	while(loop)
	{
		...
		cout<<"Detetados: "<<pointBufproj.size()<<endl;
		cout<<"Manual: "<<pointBufloc.size()<<endl;
